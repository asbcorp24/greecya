...
@php
    $u = auth()->user();
    $brandRoute = $u->role === 'director'
        ? 'admin.director.dashboard'
        : ($u->role === 'accountant' ? 'admin.finance.index' : 'admin.dashboard');
@endphp
<div class="admin-shell">
    <aside class="admin-sidebar">
        <a class="admin-brand" href="{{ route($brandRoute) }}">
            <span>Γ</span>
            <div><strong>{{ $site['site_short_name'] }}</strong><small>{{ $u->roleLabel() }}</small></div>
        </a>
        <nav>
            ...
            @endif
            @if($u->hasPermission('director.dashboard'))
                <a class="{{ request()->routeIs('admin.director.*') ? 'active' : '' }}" href="{{ route('admin.director.dashboard') }}"><i class="bi bi-speedometer2"></i>Панель директора</a>
            @endif
            @if($u->hasPermission('reception.workspace'))
                <a href="{{ route('reception.index') }}"><i class="bi bi-person-workspace"></i>Ресепшен</a>
            @endif
            @if($u->hasPermission('coach.workspace'))
                <a href="{{ route('coach.index') }}"><i class="bi bi-stopwatch"></i>Рабочее место тренера</a>
            @endif

            @if($u->hasPermission('pool.view') || $u->hasPermission('access.view') || $u->hasPermission('bookings.view') || $u->hasPermission('operations.view') || $u->hasPermission('medical.view'))
                <div class="nav-caption">Бассейн и проход</div>
            @endif
            @if($u->hasPermission('pool.view'))
                <a class="{{ request()->routeIs('admin.pool.*') ? 'active' : '' }}" href="{{ route('admin.pool.index') }}"><i class="bi bi-water"></i>Бассейн и дорожки</a>
            @endif
            @if($u->hasPermission('access.view'))
                <a class="{{ request()->routeIs('admin.access.*') ? 'active' : '' }}" href="{{ route('admin.access.index') }}"><i class="bi bi-qr-code-scan"></i>СКУД и проход</a>
            @endif
            @if($u->hasPermission('medical.view'))
                <a class="{{ request()->routeIs('admin.medical.*') ? 'active' : '' }}" href="{{ route('admin.medical.index') }}"><i class="bi bi-heart-pulse"></i>Медицинский модуль</a>
            @endif
            @if($u->hasPermission('operations.view'))
                <a class="{{ request()->routeIs('admin.operations.*') ? 'active' : '' }}" href="{{ route('admin.operations.index') }}"><i class="bi bi-activity"></i>Эксплуатация</a>
            @endif
            @if($u->hasPermission('incidents.view'))
                <a class="{{ request()->routeIs('admin.incidents.*') ? 'active' : '' }}" href="{{ route('admin.incidents.index') }}"><i class="bi bi-shield-exclamation"></i>Инциденты</a>
            @endif
            ...
            @if($u->hasPermission('customers.view'))
                <a class="{{ request()->routeIs('admin.customers.*') ? 'active' : '' }}" href="{{ route('admin.customers.index') }}"><i class="bi bi-people"></i>Клиенты 360°</a>
            @endif
            @if($u->hasPermission('families.view'))
                <a class="{{ request()->routeIs('admin.families.*') ? 'active' : '' }}" href="{{ route('admin.families.index') }}"><i class="bi bi-people-fill"></i>Семьи</a>
            @endif
            @if($u->hasPermission('swim_school.view'))
                <a class="{{ request()->routeIs('admin.swim-school.*') ? 'active' : '' }}" href="{{ route('admin.swim-school.index') }}"><i class="bi bi-person-arms-up"></i>Школа плавания</a>
            @endif
            ...
            @endif
            @if($u->hasPermission('certificates.view'))
            ...
            @if($u->hasPermission('inventory.view'))
                <a class="{{ request()->routeIs('admin.inventory.*') ? 'active' : '' }}" href="{{ route('admin.inventory.index') }}"><i class="bi bi-box-seam"></i>Склад и партии</a>
            @endif

            @if($u->hasPermission('staff.view') || $u->hasPermission('training.manage') || $u->hasPermission('content.manage'))
                <div class="nav-caption">Персонал</div>
            @endif
            @if($u->hasPermission('content.manage'))
                <a class="{{ request()->routeIs('admin.trainers.*') ? 'active' : '' }}" href="{{ route('admin.trainers.index') }}"><i class="bi bi-person-badge"></i>Тренеры</a>
            @endif
            @if($u->hasPermission('training.manage'))
            ...
            @endif
            @if($u->hasPermission('staff.view'))
                <a class="{{ request()->routeIs('admin.staff.*') ? 'active' : '' }}" href="{{ route('admin.staff.index') }}"><i class="bi bi-calendar3"></i>Графики и зарплата</a>
            @endif

            @if($u->hasPermission('reports.view') || $u->hasPermission('audit.view') || $u->hasPermission('permissions.manage'))
            ...
            @endif
            @if($u->hasPermission('audit.view'))
                <a class="{{ request()->routeIs('admin.audit.*') ? 'active' : '' }}" href="{{ route('admin.audit.index') }}"><i class="bi bi-journal-text"></i>Аудит действий</a>
            @endif
            @if($u->hasPermission('permissions.manage'))
                <a class="{{ request()->routeIs('admin.permissions.*') ? 'active' : '' }}" href="{{ route('admin.permissions.index') }}"><i class="bi bi-person-lock"></i>Роли и права</a>
            @endif
            ...
            @endif
            @if($u->hasPermission('seo.manage'))
                <a class="{{ request()->routeIs('admin.seo.*') ? 'active' : '' }}" href="{{ route('admin.seo.index') }}"><i class="bi bi-search"></i>SEO</a>
            @endif
        </nav>
        <div class="sidebar-bottom">
            <a class="{{ request()->routeIs('help.index') ? 'active' : '' }}" href="{{ route('help.index') }}"><i class="bi bi-question-circle"></i>Справка</a>
    ...
    <div class="admin-main">
        <header class="admin-header">
            ...
            <div><small>@yield('eyebrow',$site['site_name'])</small><h1>@yield('heading','CRM')</h1></div>
            <div class="admin-user">
                <span>{{ mb_substr($u->name,0,1) }}</span>
                <div><strong>{{ $u->name }}</strong><small>{{ $u->roleLabel() }}</small></div>
            </div>
        </header>

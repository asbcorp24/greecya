...
class CrmSeeder extends Seeder
{
    public function run(): void
    {
        ...
        $demo = Customer::query()->updateOrCreate(['phone'=>'+7 900 000-00-00'], ['name'=>'Демо-клиент','email'=>'client@greecya.local','birth_date'=>'1990-05-15','notes'=>'Тестовый клиент для проверки личного кабинета.','source'=>'demo','last_visit_at'=>$today->copy()->subDays(3)->setTime(18,0)]);
        $olga = Customer::query()->updateOrCreate(['phone'=>'+7 900 111-11-11'], ['name'=>'Ольга Петрова','email'=>'olga@example.com','birth_date'=>'1987-09-21','source'=>'site','last_visit_at'=>$today->copy()->subDays(8)->setTime(19,0)]);
        ...
        $maria = Customer::query()->updateOrCreate(['phone'=>'+7 900 333-33-33'], ['name'=>'Мария Кузнецова','email'=>'maria@example.com','birth_date'=>'1982-12-02','source'=>'social','last_visit_at'=>$today->copy()->subDay()->setTime(17,30)]);
        ...
        foreach([
            ['Екатерина','+7 900 444-44-44','site','Хочу записать ребёнка на обучение плаванию','new',$today->copy()->addDay()->setTime(18,0),'Перезвонить после 18:00'],
            ['Сергей','+7 900 555-55-55','phone','Интересует абонемент на 12 посещений','contacted',$today->copy()->addDays(2)->setTime(12,0),'Отправлена информация по ценам'],
            ['Алина','+7 900 666-66-66','social','Массаж и прессотерапия на выходных','qualified',$today->copy()->setTime(20,0),'Подобрать два последовательных слота'],
        ...
        foreach ($bookingData as [$uuid,$customer,$serviceSlug,$dayOffset,$people,$status,$paymentStatus,$comment]) {
            ...
            $slot = ScheduleSlot::query()->where('service_id', $service->id)->whereDate('starts_at', $today->copy()->addDays($dayOffset)->toDateString())->orderBy('starts_at')->firstOrFail();
            Booking::query()->updateOrCreate(['public_id'=>$uuid], [
                ...
                'source'=>'demo','confirmed_at'=>$status==='confirmed'?$today->copy()->setTime(10,0):null,'cancelled_at'=>null,
            ]);
        ...
        foreach ($orders as [$number,$customer,$productSlug,$paymentStatus,$status,$quantity,$externalId]) {
            ...
            $paidAt = $paymentStatus==='paid' ? $today->copy()->subDays(2)->setTime(14,0) : null;
            $order = Order::query()->updateOrCreate(['number'=>$number], [
                ...
                'promo_code'=>null,'source'=>'demo','paid_at'=>$paidAt,
            ]);
            $item = OrderItem::query()->updateOrCreate(['order_id'=>$order->id,'product_id'=>$product->id], [
                ...
                'ticket_code'=>$paymentStatus==='paid'?'T-'.$number:null,'valid_until'=>$paymentStatus==='paid'?$today->copy()->addDays($product->validity_days)->toDateString():null,
            ...
            Payment::query()->updateOrCreate(['order_id'=>$order->id,'external_id'=>$externalId], [
                ...
                'payload'=>['seeded'=>true,'order'=>$number],'paid_at'=>$paidAt,
            ]);
            ...
            if ($product->type === 'gift' && $paymentStatus === 'paid') {
                Certificate::query()->updateOrCreate(['serial'=>'GC-'.$number], [
                    ...
                    'amount'=>$product->price,'status'=>'active','valid_from'=>$today->toDateString(),'valid_until'=>$today->copy()->addDays($product->validity_days)->toDateString(),
                ]);
            }
        }
        ...
        Certificate::query()->updateOrCreate(['serial'=>'GC-DEMO-001'], [
            ...
            'amount'=>5000,'status'=>'active','valid_from'=>$today->toDateString(),'valid_until'=>$today->copy()->addMonths(6)->toDateString(),
        ]);
        Certificate::query()->updateOrCreate(['serial'=>'GC-DEMO-USED'], [
            ...
            'valid_from'=>$today->copy()->subMonths(2)->toDateString(),'valid_until'=>$today->copy()->addMonths(4)->toDateString(),
            'redeemed_at'=>$today->copy()->subDay()->setTime(17,30),'redeemed_by'=>$manager->id,
        ]);

        foreach([
            [$demo,$today->copy()->subDays(30)->setTime(10,0),'subscription','Плавание по абонементу'],
            [$demo,$today->copy()->subDays(14)->setTime(18,0),'subscription','Плавание по абонементу'],
            [$demo,$today->copy()->subDays(3)->setTime(18,0),'reception','Свободное плавание'],
            [$olga,$today->copy()->subDays(8)->setTime(19,0),'reception','Первичная тренировка'],
            [$maria,$today->copy()->subDay()->setTime(17,30),'certificate','Проход по подарочному сертификату'],
